...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	groupAccountControlRepository := repository.NewGroupAccountControlRepository(db)
	groupAccountControlService := service.NewGroupAccountControlService(groupAccountControlRepository)
	groupAccountControlHandler := admin.NewGroupAccountControlHandler(groupAccountControlService)
	...
	durableImageTaskStore := repository.NewDurableImageTaskStore(db)
	imageTaskQueue := repository.ProvideImageTaskQueue(redisClient, configConfig)
	imageTaskService := service.ProvideImageTaskService(durableImageTaskStore, imageTaskQueue, usageBillingRepository, apiKeyAuthCacheInvalidator, billingService, userGroupRateRepository, imageStorageSettingService, configConfig)
	asyncImageHandler := handler.ProvideAsyncImageHandler(imageTaskService, openAIGatewayHandler, apiKeyService, configConfig)
