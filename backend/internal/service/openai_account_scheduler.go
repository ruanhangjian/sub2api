...
// ReportOpenAIAccountScheduleResultWithSubPilot 上报调度结果并完成 SubPilot
// 失败 lease 回收。普通请求使用 ReportOpenAIAccountScheduleResult 即可。
func (s *OpenAIGatewayService) ReportOpenAIAccountScheduleResultWithSubPilot(account *Account, model string, success bool, firstTokenMs *int, observedErr error, fail SubPilotFailContext) bool {
	healthTripped := s.ReportOpenAIAccountScheduleResult(account, model, success, firstTokenMs, observedErr)
	if !success && account != nil {
		if fail.Model == "" {
			fail.Model = model
		}
		s.reportFailureForOpenAI(account.ID, fail)
	}
	return healthTripped
}

// ReportSubPilotFailure 仅回报 SubPilot 请求失败，不改变本地账号健康统计。
func (s *OpenAIGatewayService) ReportSubPilotFailure(accountID int64, fail SubPilotFailContext) {
