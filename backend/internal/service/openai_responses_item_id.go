...
func shouldStripOpenAIResponsesInputItemID(itemType, id string, stripInvalidReasoningIDOpt ...bool) bool {
	stripInvalidReasoningID := true
	if len(stripInvalidReasoningIDOpt) > 0 {
		stripInvalidReasoningID = stripInvalidReasoningIDOpt[0]
	}
	...
	if itemType == "reasoning" {
		...
		return stripInvalidReasoningID && !strings.HasPrefix(id, "rs")
	}
...
func sanitizeOpenAIResponsesInputItemIDs(body []byte, stripInvalidReasoningIDOpt ...bool) ([]byte, bool, error) {
	...
	input.ForEach(func(_, item gjson.Result) bool {
		...
		if item.IsObject() {
			...
			if id.Type == gjson.String {
				parsed.stripID = shouldStripOpenAIResponsesInputItemID(trimmedItemType, id.String(), stripInvalidReasoningID)
			}
		}
