...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	for {
		...
		if !errors.As(err, &failoverErr) {
			h.gatewayService.ReportOpenAIAccountScheduleResultWithSubPilot(account, openAIAccountScheduleModel(c, account, requestedModel, false, result), false, nil, err, service.SubPilotFailContext{Ctx: c.Request.Context(), GroupID: apiKey.GroupID, Model: requestedModel})
