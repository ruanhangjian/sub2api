			...
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					if failoverErr.ShouldReportAccountScheduleFailure() {
						h.gatewayService.ReportOpenAIAccountScheduleResultWithSubPilot(account, openAIAccountScheduleModel(c, account, reqModel, false, nil), false, nil, err, service.SubPilotFailContext{Ctx: c.Request.Context(), GroupID: apiKey.GroupID, Model: reqModel})
					}
