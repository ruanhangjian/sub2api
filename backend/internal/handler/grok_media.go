		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if failoverErr.ShouldReportAccountScheduleFailure() {
					h.gatewayService.ReportOpenAIAccountScheduleResultWithSubPilot(account, grokMediaScheduleModel(account, routingModel, nil), false, nil, err, service.SubPilotFailContext{Ctx: c.Request.Context(), GroupID: apiKey.GroupID, Model: requestModel})
				}
