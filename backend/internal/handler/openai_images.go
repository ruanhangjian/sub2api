...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	for {
		...
		applySubPilotLease(c, selection.SubPilotLeaseID)
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &imageUpstreamErr) {
					...
					h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, !retryableServerError, nil, service.SubPilotFailContext{Ctx: c.Request.Context(), GroupID: apiKey.GroupID, Model: requestModel})
				...
				if errors.As(err, &failoverErr) {
					h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, false, nil, service.SubPilotFailContext{Ctx: c.Request.Context(), GroupID: apiKey.GroupID, Model: requestModel})
		...
		if result != nil {
			...
			h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, true, result.FirstTokenMs, service.SubPilotFailContext{Ctx: c.Request.Context(), GroupID: apiKey.GroupID, Model: requestModel})
		} else {
			h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, true, nil, service.SubPilotFailContext{Ctx: c.Request.Context(), GroupID: apiKey.GroupID, Model: requestModel})
		}
