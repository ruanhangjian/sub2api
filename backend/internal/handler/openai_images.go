			...
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &imageUpstreamErr) {
					...
					if retryableServerError {
						h.gatewayService.ReportOpenAIAccountScheduleResultWithSubPilot(account, openAIAccountScheduleModel(c, account, requestModel, false, result), false, nil, err, service.SubPilotFailContext{Ctx: c.Request.Context(), GroupID: apiKey.GroupID, Model: requestModel})
					} else {
						...
						h.gatewayService.ReportSubPilotFailure(account.ID, service.SubPilotFailContext{Ctx: c.Request.Context(), GroupID: apiKey.GroupID, Model: requestModel})
					}
